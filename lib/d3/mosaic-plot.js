...
const defaultOptions = {
  levelField: 'metrics.qualification-level',
  learningModeField: 'metrics.learning-mode.mode',
  areaField: 'metrics.area',
  width: 900,
  height: 900,
  fillColor: 'steelblue',
...
/*!
Referencias: example: https://photino.github.io/d3-components/api/mosaic-plot.html
             npm package: https://www.npmjs.com/package/d3-components"
*/
...
d3.mosaicPlot = function (data, options) {
  ...
  if (!uniformX) {
    ...
    columns.forEach(function (column, index) {
      ...
      // console.log(column, columnValues[index]);
  ...
  if (renderer === 'svg') {
    ...
    // var step = x.step();
    // console.log(step);
    // columnMap.values().forEach(c => {
    //   console.log(step + " + " + c.offset + " = " + (step + c.offset));
    // })
    // // Add the x Axis
    // g.append("g")
...
function adjustData(data, opts) {
  var level = ['-','NIVEL 1', 'NIVEL 2', 'NIVEL 3', 'NIVEL 4', 'NIVEL 5'];
  var mod = ['PRESENCIAL', 'ONLINE', 'B-LEARNING'];
  var area = ['Administración',
  ...
  data.forEach(d => {
    count = 0;
    lev = "", mode="", are="";
    lev = _.get(d, opts.levelField);
    mode = _.get(d, opts.learningModeField);
    are =_.get(d, opts.areaField);
    for (let i = 0; i < level.length; i++) {
      for (let j = 0; j < mod.length; j++) {
        for (let k = 0; k < area.length; k++) {
          if (_.isEqual(lev, level[i])  && _.isEqual(mode, mod[j])  && _.isEqual(are, area[k]) ){
            count = count + 1;
          }
        }
      }     
...
function chart(querySelector, data, opts) {
  ...
  var adjustedData = adjustData(data, finalOptions);


  try {
    const data = require('./titanic-passengers.json');
    // const dataPie = require('./browser-user-share.json');
    console.log(d3);

    // d3.pieChart(dataPie,{id: 'pie-plot-default'});
    d3.mosaicPlot(adjustedData, {
      id: 'canvas',
      columns: ['-', 'NIVEL 1', 'NIVEL 2', 'NIVEL 3', 'NIVEL 4', 'NIVEL 5'],
      rows: ['PRESENCIAL', 'ONLINE', 'B-LEARNING'],
      // scaleZ: {
      //   paddingMidst: 0.02
      // },
      labels: {
