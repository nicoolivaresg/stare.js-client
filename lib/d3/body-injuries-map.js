...
function showTooltip(d, opts) {
    ...
    if(d.data.length > 0){
        html = html + `<div><b>Mín ddo 1ra instancia:</b>${d.resume.first.min}</div>`;
        html = html + `<div><b>Max ddo 1ra instancia:</b>${d.resume.first.max}</div>`;
        html = html + `<div><b>Promedio ddo 1ra instancia:</b>${d.resume.first.average}</div>`;
        html = html + `<br/>`;
    
        html = html + `<div><b>Mín ddo total:</b>${d.resume.full.min}</div>`;
        html = html + `<div><b>Max ddo total:</b>${d.resume.full.max}</div>`;
        html = html + `<div><b>Promedio ddo:</b>${d.resume.full.average}</div>`;
        html = html + `<br/>`;
...
function assignDataToAreas(mapData, data, opts) {
    mapData.map(a => {
        ...
        if(a.data.length>0){
            var amountsFirst = a.data.map(doc => { return parseInt(_.get(doc, opts.amountFirstField), 10) });
            ...
            var amountsFull = a.data.map(doc => { return parseInt(_.get(doc, opts.amountFullField), 10) });
