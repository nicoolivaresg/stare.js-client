...
function showResults(documents, opts) {
    ...
    var montoOtorgadoPrimeraInstancia = results.append('div')
        ...
        .append('p')
        .text((d)=>{
            return  'Primera instancia: $' + _.get(d,opts.amountGivenFirstField );
        });
    ...
    .style('align-items','center')
    .style('text-align','center')
    .html('<i class="fas fa-file fa-lg"></i>')
    .style('height','30px')
    .on('click',(d)=>{
        ...
        function ready(values) {
            ...
            let amounts = filteredData.map(d => {
                ...
                switch (finalOptions.fillWith) {
                    case 'primera':
                        return _.get(d, finalOptions.amountGivenFirstField) / divisor;
                    case 'demandado':
                        return _.get(d, finalOptions.amountAskedFullField) / divisor;
                    default:
                        return _.get(d, finalOptions.amountGivenFirstField) / divisor;
                }
            });
