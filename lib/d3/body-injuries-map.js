...
function createResultsContainer(params) {
    resultsContainer =
        d3.select('body')
            .append('div')
...
function showResults(documents, opts) {
    ...
    //TODO: Rellenar contenedor de resultados
    resultsContainer.html("<div>Algunos documentos acá</div>");
}
...
function removeAll() {
    console.log('Remove all children from mainContainer');
    // mainContainer.html("");
...
function chart(querySelector, data, opts) {
    ...
    mainContainer = d3.select(querySelector)
        ...
        function onchange() {
            ...
            console.log('Re-rendering on filter ', categorySelected);
    ...
    function render(data, filters) {
        ...
        function ready(values) {
            ...
            //TODO: Capturar la escala de colores según todos los montos 
            ...
            // console.log(`Filtered with ${categorySelected}  ${filteredData.length}`);
            // console.log(front, back, unsegmented);
           ...
           frontImgContainer.append('map')
            ...
            legendsContainer.append('h5')
                .attr('x', widthLegend + 20)
                .attr('y', barHeight / 2)
                .attr("dy", ".35em")
                .text(`${_.isEqual(finalOptions.unit, 'uf') ? "Unidades de Fomento (UF)" :
                    "Pesos chilenos (CLP)"
                    }`);
