    ...
    if (juris.data != null) {
        ...
        if (!_.isEqual(opts.unit, 'uf')) {
            results.append('div')
                ...
                .append('p')
                .text((d) => {
                    return `Monto ${_.isEqual(opts.fillWith, 'full') ? "demandado" : "otorgado 1ra instancia"}  (valor año ${new Date(_.get(d, opts.resultsFirstSentenceDateField)).getFullYear()} en pesos chilenos): $${
                        _.isEqual(opts.fillWith, 'full') ? 
            ...
            results.append('div')
                ...
                .append('p')
                .text((d) => {
                    return `Monto ${_.isEqual(opts.fillWith, 'full') ? "demandado" : "otorgado 1ra instancia"} (valor año ${new Date().getFullYear()} en pesos chilenos): $${
                        _.isEqual(opts.fillWith, 'full') ?
        ...
        if (_.isEqual(opts.unit, 'uf')) {
            results.append('div')
                ...
                .append('p')
                .text((d) => {
                    return `Monto ${_.isEqual(opts.fillWith, 'full') ? "demandado" : "otorgado 1ra instancia"}  (valor año ${new Date(_.get(d, opts.resultsFirstSentenceDateField)).getFullYear()} en UF): ${
                        _.isEqual(opts.fillWith, 'full') ?
            ...
            results.append('div')
                ...
                .append('p')
                .text((d) => {
                    return `Monto ${_.isEqual(opts.fillWith, 'full') ? "demandado" : "otorgado 1ra instancia"}  (valor año ${new Date().getFullYear()} en UF): ${
                        _.isEqual(opts.fillWith, 'full') ? 
...
function showDynamicCard(juris, opts) {
    ...
    if (_.get(juris, 'data')) {
        dynamicCard
            ...
            .style('width', '100%')
            .style('padding', '2%')
            .text(`Monto promedio ${_.isEqual(opts.fillWith, 'full') ? "demandado" : "otorgado 1ra instancia"}  (valor presente ${new Date().getFullYear()}):  ${_.isEqual(opts.unit, 'uf') ?
                    _.get(juris, 'properties.average.' + opts.fillWith) + " UF" :
                    "$" + _.get(juris, 'properties.average.' + opts.fillWith)
                }`);
    }
        ...
        if (_.has(e, 'data')) {
            let amountsGiven = e.data.map(doc => {
                return _.isEqual(opts.unit, 'uf') ?
                    parseInt(_.get(doc, opts.amountGivenFirstPresentUfField)) :
                    parseInt(_.get(doc, opts.amountGivenFirstPresentField))
            });
            var sum = amountsGiven.reduce((a, b) => a + b, 0);
            var averageFirst = (sum / amountsGiven.length) || 0;
            let amountsAsked = e.data.map(doc => {
                return _.isEqual(opts.unit, 'uf') ?
                    parseInt(_.get(doc, opts.amountAskedFullPresentUfField)) :
                    parseInt(_.get(doc, opts.amountAskedFullPresentField))
            });
