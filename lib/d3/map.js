...
const defaultOptions = {
  ...
  filledOpacity : 0.8,
  fillWith: 'first',
  colorInterpolation: 'interpolateRdYlGn',
...
function chart(querySelector, data, opts) {
  ...
  function ready(topojsonFilesLoaded) {
    ...
    let fullAmounts = data.map(d => _.get(d, finalOptions.jurisDataField +'.'+ finalOptions.amountFullField) / finalOptions.uf );
    fullAmounts = fullAmounts.sort((a, b) => a - b);
